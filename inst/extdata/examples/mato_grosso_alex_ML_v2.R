...
#----------------------------------------------------------------------------------
#----------- running the functions for k-fold validation one by one
#----------------------------------------------------------------------------------

    folds <- 2

    #extract the bands to be included in the patterns
    data.tb <- sits_select(embrapa_mt.tb, bands)
    ...
    data_train_band.tb <- sits_select(embrapa_mt.tb, bands[iband])

    # use the extracted partition to create the patterns
    patterns.tb <- sits_gam(data_train_band.tb)

    # find the matches on the training data
    distances_train.tb <- sits_TWDTW_distances (data_train_band.tb, patterns.tb)
    distances_train.tb

    # find the distances in the test data
    distances_test.tb  <- sits_TWDTW_distances (data_test.tb, patterns.tb)
    distances_test.tb

    # fit a model on the training data set
    model.ml <- sits_svm (distances_train.tb, cost = 100, epsilon = 0.1)
    model.ml <- sits_lda (distances_train.tb)
    ...
    predict.tb1$predicted <- as.character(newlabels2.lst[predict.tb$predicted])
    predict.tb1$label <- as.character(newlabels2.lst[predict.tb$label])

    caret::confusionMatrix(predict.tb1$predicted, predict.tb1$label)

#----------------------------------------------------------------------------------
#------------ end of k-validation emulation
#----------------------------------------------------------------------------------
